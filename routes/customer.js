...
router.get('/details/:vmId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    const querystr =  "(select am.am_invoice_no as invoice,am.am_date as date,am.am_total_amount as debit,0 as credit,'Airline' as type, '1' as num from airline_master am LEFT OUTER JOIN customer_master cm on am.am_cm_id=cm.cm_id where am.am_status=0 and cm.cm_id = $1) UNION "+
                      "(select vm.vm_invoice_no as invoice,vm.vm_date as date,vm.vm_total_amount as debit,0 as credit,'Visa' as type, '1' as num from visa_master vm LEFT OUTER JOIN customer_master cm on vm.vm_cm_id=cm.cm_id where vm.vm_status=0 and cm.cm_id = $1) UNION "+
                      "(select hm.hm_invoice_no as invoice,hm.hm_date as date,hm.hm_total_amount as debit,0 as credit,'Hotel' as type, '1' as num from hotel_master hm LEFT OUTER JOIN customer_master cm on hm.hm_cm_id=cm.cm_id where hm.hm_status=0 and cm.cm_id = $1) UNION "+
